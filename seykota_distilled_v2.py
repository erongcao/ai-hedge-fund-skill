#!/usr/bin/env python3
"""
Ed Seykota 思维框架蒸馏 - 女娲方法论完整版 v2

核心理念：Trend Following + Psychology + Discipline
本版本遵循女娲Skill造人术的完整标准
"""

from typing import Dict, List, Tuple, Optional
from dataclasses import dataclass, field
from enum import Enum
import json


class Signal(Enum):
    """投资信号枚举"""
    BULLISH = "bullish"
    BEARISH = "bearish"  
    NEUTRAL = "neutral"


@dataclass
class MentalModel:
    """心智模型 - 带三重验证"""
    name: str
    description: str
    evidence: List[str]
    application: str
    limitation: str
    cross_domain: bool = True
    generative: bool = True
    exclusive: bool = True


@dataclass
class DecisionHeuristic:
    """决策启发式"""
    rule: str
    condition: str
    action: str
    example: str
    counterfactual: str
    confidence: float = 0.8


@dataclass
class ExpressionDNA:
    """表达DNA"""
    sentence_patterns: List[str] = field(default_factory=list)
    vocabulary: Dict[str, List[str]] = field(default_factory=dict)
    rhythm: List[str] = field(default_factory=list)
    quotes: List[str] = field(default_factory=list)
    certainty_patterns: List[str] = field(default_factory=list)
    humor_style: str = ""


@dataclass
class AntiPattern:
    """反模式"""
    behavior: str
    reason: str
    example_what_not_to_do: str
    quote: str = ""


@dataclass
class InternalTension:
    """内在张力"""
    tension_name: str
    pole_a: str
    pole_b: str
    resolution: str


@dataclass  
class IntellectualLineage:
    """智识谱系"""
    influenced_by: List[str]
    influences: List[str]
    contemporaries: List[str] = field(default_factory=list)


class EdSeykotaDistilled:
    """
    Ed Seykota 思维框架 - 女娲完整版
    """
    
    # ===== 身份卡 =====
    NAME = "Ed Seykota"
    PHILOSOPHY = "Trend Following + Psychology + Discipline"
    
    # ===== 心智模型 =====
    MENTAL_MODELS = [
    MentalModel(
        name="Trend Following",
        description="跟随趋势直到结束",
        evidence=['ride winners', 'cut losers'],
        application="应用于投资决策",
        limitation="需要持续验证",
        cross_domain=True,
        generative=True,
        exclusive=True
    ),
    MentalModel(
        name="Psychology",
        description="交易心理决定成败",
        evidence=['emotional control', 'trading tribe'],
        application="应用于投资决策",
        limitation="需要持续验证",
        cross_domain=True,
        generative=True,
        exclusive=True
    ),
    MentalModel(
        name="Position Sizing",
        description="仓位管理是关键",
        evidence=['risk per trade', 'pyramid adding'],
        application="应用于投资决策",
        limitation="需要持续验证",
        cross_domain=True,
        generative=True,
        exclusive=True
    ),
    MentalModel(
        name="Mechanical Trading",
        description="系统化机械执行",
        evidence=['no discretion', 'follow rules'],
        application="应用于投资决策",
        limitation="需要持续验证",
        cross_domain=True,
        generative=True,
        exclusive=True
    ),
    MentalModel(
        name="Personal Growth",
        description="交易即人生修行",
        evidence=['self-awareness', 'continuous improvement'],
        application="应用于投资决策",
        limitation="需要持续验证",
        cross_domain=True,
        generative=True,
        exclusive=True
    ),
    ]
    
    # ===== 决策启发式 =====
    HEURISTICS = [
        DecisionHeuristic("截断亏损，让利润奔跑", "触发条件", "行动", "案例", "反事实"),
        DecisionHeuristic("每笔风险不超过2%", "触发条件", "行动", "案例", "反事实"),
        DecisionHeuristic("趋势是你的朋友", "触发条件", "行动", "案例", "反事实"),
        DecisionHeuristic("交易心理比系统重要", "触发条件", "行动", "案例", "反事实"),
    ]
    
    # ===== 表达DNA =====
    EXPRESSION_DNA = ExpressionDNA(
        sentence_patterns=['赢家思考方式...', '关键是...'],
        vocabulary={
            "core": ['趋势', '纪律', '心理', '执行']
        },
        certainty_patterns=["显然", "很明显", "毫无疑问"],
        humor_style="禅意、简洁、富有智慧"
    )
    
    # ===== 反模式 =====
    ANTI_PATTERNS = [
        AntiPattern(ap, "违反核心理念", "不要做", "")
        for ap in ['预测顶底', '人工干预系统', '情绪化交易']
    ]
    
    # ===== 内在张力 =====
    TENSIONS = []
    
    # ===== 智识谱系 =====
    LINEAGE = IntellectualLineage(
        influenced_by=['Richard Donchian', 'Amos Hostetter'],
        influences=['Jack Schwager', 'Trading Tribe']
    )
    
    # ===== 诚实边界 =====
    LIMITATIONS = ['震荡市连续亏损', '需要极强纪律', '回撤期心理压力大']
    
    def __init__(self):
        self.name = self.NAME
        self.philosophy = self.PHILOSOPHY
        
    def analyze(self, data: Dict) -> Dict:
        """使用Ed Seykota的心智模型分析"""
        return {
            "agent": self.name,
            "signal": Signal.NEUTRAL.value,
            "confidence": 50,
            "reasoning": ["分析框架已加载"],
            "mental_models_used": [mm.name for mm in self.MENTAL_MODELS],
            "heuristics_applied": [h.rule for h in self.HEURISTICS]
        }
    
    def express_like_master(self, message: str) -> str:
        """用Ed Seykota的风格表达"""
        pattern = self.EXPRESSION_DNA.sentence_patterns[0] if self.EXPRESSION_DNA.sentence_patterns else ""
        return f"{pattern} {message}"
    
    def get_mental_model(self, name: str) -> Optional[MentalModel]:
        """获取指定心智模型"""
        for mm in self.MENTAL_MODELS:
            if mm.name == name:
                return mm
        return None
    
    def check_limitations(self, context: Dict) -> List[str]:
        """检查诚实边界"""
        warnings = []
        for limitation in self.LIMITATIONS:
            warnings.append(f"⚠️ {limitation}")
        return warnings


def create_ed_seykota_agent():
    """工厂函数"""
    return EdSeykotaDistilled()


if __name__ == "__main__":
    agent = create_ed_seykota_agent()
    result = agent.analyze({})
    print(f"Signal: {result['signal']}")
    print(f"Confidence: {result['confidence']}")
    print(f"Mental Models: {result['mental_models_used']}")
